...
sys.path.append('..')
sys.path.append('../..')
from model.networks.segformer import *
...
class BridgeLayer_4(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if (type(inputs) == list):
            ...
            c1, c2, c3, c4 = inputs
            B, C, _, _= c1.shape
            c1f = c1.permute(0, 2, 3, 1).reshape(B, -1, C)  # 1024*64
            c2f = c2.permute(0, 2, 3, 1).reshape(B, -1, C)  # 512*64
            c3f = c3.permute(0, 2, 3, 1).reshape(B, -1, C)  # 320*64
            c4f = c4.permute(0, 2, 3, 1).reshape(B, -1, C)  # 128*64
        ...
        tem1 = tx[:,:1024,:].reshape(B, -1, C) 
        tem2 = tx[:,1024:1536,:].reshape(B, -1, C*2)
        tem3 = tx[:,1536: 1856,:].reshape(B, -1, C*5)
        tem4 = tx[:,1856 : , :].reshape(B, -1, C*8)

        m1f = self.mixffn1(tem1, 32, 32).reshape(B, -1, C)
        m2f = self.mixffn2(tem2, 16, 16).reshape(B, -1, C)
        m3f = self.mixffn3(tem3, 8, 8).reshape(B, -1, C)
        m4f = self.mixffn4(tem4, 4, 4).reshape(B, -1, C)
...
class BridegeBlock_4(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sk1 = bridge4[:,:1024,:].reshape(B, 32, 32, C).permute(0,3,1,2) 
        sk2 = bridge4[:,1024:1536,:].reshape(B, 16, 16, C*2).permute(0,3,1,2) 
        sk3 = bridge4[:,1536:1856,:].reshape(B, 8, 8, C*5).permute(0,3,1,2) 
        sk4 = bridge4[:,1856:,:].reshape(B, 4, 4, C*8).permute(0,3,1,2) 
...
class MISSFormer(nn.Module):
    def __init__(self, num_classes=5, token_mlp_mode="mix_skip", encoder_pretrained=True):
        super().__init__()
        ...
        self.backbone = MiT(128, dims, layers,token_mlp_mode)

        self.reduction_ratios = [1, 2, 4, 8]
        self.bridge = BridegeBlock_4(64, 1, self.reduction_ratios)

        self.decoder_3= MyDecoderLayer((4,4), in_out_chan[3], heads[3], reduction_ratios[3],token_mlp_mode, n_class=num_classes)
        self.decoder_2= MyDecoderLayer((8,8),in_out_chan[2], heads[2], reduction_ratios[2], token_mlp_mode, n_class=num_classes)
        self.decoder_1= MyDecoderLayer((16,16), in_out_chan[1], heads[1], reduction_ratios[1], token_mlp_mode, n_class=num_classes)
        self.decoder_0= MyDecoderLayer((32,32), in_out_chan[0], heads[0], reduction_ratios[0], token_mlp_mode, n_class=num_classes, is_last=True)
    ...
    def forward(self, x):
        ...
        encoder = self.backbone(x) # [bs, 64, 32, 32], [bs, 128, 16, 16], [8, 320, 8, 8], [8, 512, 4, 4]
...
if __name__ == "__main__":
    ...
    oup = miss(X)
    print(oup.shape)
    flops, params = thop.profile(miss, inputs = (X, ))
    print(flops / (10 ** 9))
    print(params)