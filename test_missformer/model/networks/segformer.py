...
class Scale_reduce(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        print(x.shape)
        if(len(self.reduction_ratio)==4):
            tem0 = x[:,:1024,:].reshape(B, 32, 32, C).permute(0, 3, 1, 2) 
            tem1 = x[:,1024:1536,:]
            print(tem1.shape)
            tem1.reshape(B, 16, 16, C*2).permute(0, 3, 1, 2)
            tem2 = x[:,1536:1792,:].reshape(B, 14, 14, C*5).permute(0, 3, 1, 2)
            tem3 = x[:,1792:1856,:]
...
class MiT(nn.Module):
    def __init__(self, image_size, dims = [64, 128, 320, 512], layers = [2, 2, 2, 2], token_mlp='mix_skip'):
        super().__init__()
        patch_sizes = [7, 3, 3, 3]
        strides = [4, 2, 2, 2]
