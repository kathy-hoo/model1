...
import os 
# from model.networks.vision_transformer  import SwinUnet as  Model
from model.networks.MISSFormer  import MISSFormer as  Model
import random
...
# from model.vit_seg_configs import get_b16_configtrain as get_config
MAX_EPOCHS = 300
LEARNING_RATE = 1e-4
...
if __name__ == "__main__":
    mode = "test"
    check_point_path = '/home/kathy/projects/project_guo/test_missformer/logs/logger_train2-missformer/version_6/checkpoints/epoch=57-step=17400.ckpt'
    # check_point_path = '/home/kathy/projects/project_guo/test_transBts/logs/logger_train2/version_0/checkpoints/epoch=93-step=5734.ckpt'
    train_loader, val_laoder = get_dataloader(batch_size=2)
    if mode == "train":
        info = '''仅CE loss '''

        model = TrainModel(info)
        
        logger = CSVLogger("logs", name="logger_train2-missformer")
        trainer = pl.Trainer(accelerator="gpu", devices=[5], precision=16, max_epochs=MAX_EPOCHS, logger=logger)
        trainer.fit(model, train_loader)
    elif mode == "test":
        logger = CSVLogger("logs", name="logger_test-swin(ad)")
        model = TrainModel.load_from_checkpoint(checkpoint_path=check_point_path, map_location=torch.device("cuda:1"))
        trainer = pl.Trainer(accelerator="gpu", devices=[1], precision=16, max_epochs=MAX_EPOCHS, logger=logger)
        
        trainer.test(model, val_laoder)
    else:
        model = TrainModel()
        X = torch.rand(16, 1, 128, 128).to(model.device)
        print(model(X).shape )
