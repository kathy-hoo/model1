...
if __name__ == "__main__":
    mode = "test"
    check_point_path = '/home/kathy/projects/project_guo/test_unet/logs/train_a/version_1/checkpoints/epoch=106-step=32100.ckpt'
    ...
    if mode == "train":
        ...
        trainer = pl.Trainer(accelerator="gpu", devices=[9], precision=16, max_epochs=MAX_EPOCHS, logger=logger)
        trainer.fit(model, train_loader)
    elif mode == "test":
        logger = CSVLogger("logs", name="test_a")
        model = TrainModel.load_from_checkpoint(checkpoint_path=check_point_path, map_location=torch.device("cuda:9"))
        trainer = pl.Trainer(accelerator="gpu", devices=[9], precision=16, max_epochs=MAX_EPOCHS, logger=logger)
