...
class MiddleModule(nn.Module):
    def __init__(self, proj_dim = 64):
        super().__init__()
        self.x5_dem_1 = nn.Sequential(nn.Conv2d(512 , proj_dim, kernel_size=3, padding=1), nn.BatchNorm2d(proj_dim), nn.ReLU(inplace=True))
        self.x4_dem_1 = nn.Sequential(nn.Conv2d(256 , proj_dim, kernel_size=3, padding=1), nn.BatchNorm2d(proj_dim), nn.ReLU(inplace=True))
        self.x3_dem_1 = nn.Sequential(nn.Conv2d(128 , proj_dim, kernel_size=3, padding=1), nn.BatchNorm2d(proj_dim), nn.ReLU(inplace=True))
        self.x2_dem_1 = nn.Sequential(nn.Conv2d(64 , proj_dim, kernel_size=3, padding=1), nn.BatchNorm2d(proj_dim), nn.ReLU(inplace=True))
        ...
        self.x5_dem_5 = nn.Sequential(nn.Conv2d(512 , proj_dim, kernel_size=3, padding=1), nn.BatchNorm2d(proj_dim),
                                      nn.ReLU(inplace=True))
